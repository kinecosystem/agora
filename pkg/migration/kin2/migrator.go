...
var (
	onDemandSuccessCounter = migration.OnDemandSuccessCounterVec.WithLabelValues("2")
	onDemandFailureCounter = migration.OnDemandFailureCounterVec.WithLabelValues("2")
)
...
func (m *kin2Migrator) GetMigrationAccount(ctx context.Context, account ed25519.PublicKey) (ed25519.PublicKey, error) {
	migrationAccount, migrationAccountKey, err := migration.DeriveMigrationAccount(account, m.migrationSecret)
	if err != nil {
		return nil, err
	}
