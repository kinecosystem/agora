...
func (m *kin3Migrator) migrateAccount(ctx context.Context, info accountInfo, commitment solana.Commitment) (err error) {
	...
	window := (20 * time.Second) * time.Duration(1+rand.Float64()/2)
...
func (m *kin3Migrator) recover(ctx context.Context, account, migrationAccount ed25519.PublicKey, commitment solana.Commitment) error {
	...
	sigStatus, err := m.sc.GetSignatureStatus(state.Signature, commitment)
	if err == solana.ErrSignatureNotFound {
		// reset the state, and trigger the migration again.
		//
		// note: InitiateMigration() does account existence checks, so we don't
		//       have to in the recovery stage.
		if err := m.store.Update(ctx, account, state, migration.ZeroState); err != nil {
		...
		return errors.Wrap(err, "failed to check signature status")
	}

	if sigStatus.ErrorResult != nil {
		// For now, just reset the state and propagate the error back up.
		// In the case that the account already exists, the retry will
		// trigger another InitiateMigration(), which already handles the case.
		if err := m.store.Update(ctx, account, state, migration.ZeroState); err != nil {
			log.WithError(err).Warn("Failed to reset state during recover()")
		}

		log.WithError(err).Info("Migration transaction failed; resetting")
		return sigStatus.ErrorResult
	}

	if sigStatus.Confirmations == nil {
		return migration.MarkComplete(ctx, m.store, account, state)
	}
