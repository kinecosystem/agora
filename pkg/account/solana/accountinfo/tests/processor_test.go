...
type testEnv struct {
	store     accountinfo.StateStore
	processor *accountinfo.StateProcessor
...
func setup(t *testing.T) (env testEnv) {
	...
	env.processor = accountinfo.NewStateProcessor(
		env.store,
		env.sc,
		env.tc,
	)
...
func TestProcess_Existing(t *testing.T) {
	...
	require.NoError(t, env.store.Put(context.Background(), &accountinfo.State{
		Account: accounts[0],
		Owner:   accounts[1],
		Balance: 20,
		Slot:    3,
