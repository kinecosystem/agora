...
type testCase func(t *testing.T, store info.StateStore, teardown func())

func RunStateStoreTests(t *testing.T, store info.StateStore, teardown func()) {
	for _, test := range []testCase{testStoreRoundTrip, testGetOwnersByAccount} {
		test(t, store, teardown)
	}
}

func testStoreRoundTrip(t *testing.T, store info.StateStore, teardown func()) {
	t.Run("testRoundTrip", func(t *testing.T) {
		...
		assert.Equal(t, info.ErrNotFound, err)
		assert.Nil(t, state)

		require.NoError(t, store.Put(context.Background(), &info.State{
			Account: accounts[0],
			Owner:   owners[0],
			Balance: balances[0],
			Slot:    slot,
...
func testGetOwnersByAccount(t *testing.T, store info.StateStore, teardown func()) {
